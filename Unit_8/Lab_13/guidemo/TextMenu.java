...
public class TextMenu extends JMenu {
	...
	private JRadioButtonMenuItem left; // controls whether the text is left align or not.
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem height = new JMenuItem("Set Line Spacing...");
		height.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentHeight = panel.getTextItem().getLineHeightMultiplier();
				String s = JOptionPane.showInputDialog(panel, "Multipy the default line spacing by \n"
						+ "what amount?", currentHeight);
				if (s != null && s.trim().length() > 0) {
					try {
	...
	/**
	 * Create a menu containing available methods to justify.
	 */
